...
class ECG_CNN_Echo_CNN_Fusion(nn.Module):
    def __init__(self):
        ...
        # Echo CNN
        self.echo_cnn = models.resnext101_32x8d(pretrained=True)

        self.ecg_cnn = ECG_CNN(num_conv_layers=6, first_layer_filters=128, kernel_size=7, dropout_rate=0.10)
    ...
    def forward(self, echo_input, ecg_input):
        ...
        echo_features = self.echo_cnn(echo_input) # 1000

        # downsample Echo features to 64 to match ECG output
        echo_down_feats = self.fc_layers(echo_features)

        ecg_features = self.ecg_cnn(ecg_input) # dataloader --> use the load ECG function, just use the first 2500 samples
